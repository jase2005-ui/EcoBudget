    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm space-y-6">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Sign in to EcoBudget</p>
        </div>

        <form onSubmit={handleSubmit} className="bg-card border border-border rounded-2xl p-6 space-y-4">
          {error && (
            <div data-testid="login-error" className="bg-destructive/10 text-destructive text-sm px-4 py-3 rounded-xl font-medium">
              {error}
            </div>
          )}
          ...
          <Button data-testid="login-submit" type="submit" className="w-full rounded-xl h-11 font-semibold" disabled={loading}>
        ...
        <p className="text-center text-sm text-muted-foreground">
          ...
          <Link to="/register" data-testid="go-to-register" className="text-primary font-semibold hover:underline">Sign up</Link>
        </p>
      </div>
    </div>
  );
